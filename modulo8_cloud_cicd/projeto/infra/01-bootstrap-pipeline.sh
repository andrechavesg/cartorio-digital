...
ARTIFACT_BUCKET="cartorio-${ENVIRONMENT}-cicd-artifacts"
PIPELINE_NAME="cartorio-${ENVIRONMENT}-pipeline"
GITHUB_OWNER="${GITHUB_OWNER:?Informe GITHUB_OWNER}"
GITHUB_REPO="${GITHUB_REPO:-cartorio-digital}"
GITHUB_TOKEN="${GITHUB_TOKEN:?Informe GITHUB_TOKEN com acesso ao repositório}"
    ...
    \"stages\": [
      {\"name\": \"Source\", \"actions\": [
        {\"name\": \"GitHubSource\",\"actionTypeId\": {\"category\": \"Source\",\"owner\": \"ThirdParty\",\"provider\": \"GitHub\",\"version\": \"1\"},
         ...
         \"configuration\": {\"Owner\": \"${GITHUB_OWNER}\",\"Repo\": \"${GITHUB_REPO}\",\"Branch\": \"main\",\"OAuthToken\": \"${GITHUB_TOKEN}\"}}
      ]},
      {\"name\": \"Build\", \"actions\": [
...
aws codepipeline create-pipeline --cli-input-json "${PIPELINE_JSON}" 2>/dev/null || \
